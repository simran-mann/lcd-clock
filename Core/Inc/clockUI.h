...
#ifndef INC_CLOCKUI_H_
#define INC_CLOCKUI_H_
...
typedef enum {
    E_SETTING, //when the settings option is clicked
    E_ROOT_BACK,
...
#define  UI_STATE_CHANGE(s)   (clk->state = s)
...
typedef struct {
    Clock clock_inst;
    Lvgl lvgl_inst;//for touchscreen and lcd library
    UI_state_t state;
    Clock usr_setting;
    Clock setting_save;
} ClockUI;
